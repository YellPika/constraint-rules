...
findRules ∷ (Definitions, Equalities) ⇒ [Ct] → TcPluginM ([Rule], [Ct])
findRules givens = do
...
findIgnored ∷ (Definitions, Equalities) ⇒ [Ct] → TcPluginM [(RuleUsage, Name)]
findIgnored = mapMaybeM (go . ctPred) where
  ...
  go t@(IgnoreTy _) = do
    addErr (hang "Malformatted Ignore constraint:" 4 (ppr t))
    return Nothing
  go _ = return Nothing

findSpecifiedRules ∷ (Definitions, Equalities) ⇒ [Ct] → TcPluginM ([Rule], [Ct])
findSpecifiedRules = loop where
  ...
  go (ctPred → t@(UseTy _)) = do
    addErr (hang "Malformatted Use constraint:" 4 (ppr t))
    return Nothing
  go _ = return Nothing

findDefaultRules ∷ Definitions ⇒ TcPluginM [Rule]
findDefaultRules = do
  topEnv ← getTopEnv
...
makeRule ∷ Definitions ⇒ Maybe GlobalRdrElt → CtLoc → RuleUsage → Name → TcPluginM (Maybe Rule)
makeRule ruleElt ruleLoc kind name = runMaybeT do
  ruleDef ← MaybeT (lookupVar name)
  (ruleVars, ruleCts, goalTy) ← MaybeT (return (parseRuleType (varType ruleDef)))
  let ruleArgs = emptyTCvSubst
  ruleGoal ← MaybeT (parseRuleGoal name kind goalTy)
  return Rule {..}

parseRuleType ∷ Definitions ⇒ Type → Maybe ([Var], [Type], Type)
parseRuleType (tcSplitForAllTys → (vars, tcSplitPhiTy → (cts, DictTy [t]))) = Just (vars, cts, t)
parseRuleType _ = Nothing

parseRuleGoal ∷ Definitions ⇒ Name → RuleUsage → Type → TcPluginM (Maybe RuleGoal)
...
parseRuleGoal name kind goal = do
  addErr (hang ("Rule" <+> quotes (ppr name) <+> "has invalid" <+> ppr kind <+> "goal:") 4 (ppr goal))
  return Nothing

checkArgs ∷ Equalities ⇒ TCvSubst → Type → [Var] → [Type] → MaybeT TcPluginM ()
...
lookupName ∷ FastString → FastString → TcPluginM (Maybe Name)
lookupName mdName occ = do
  ...
  case result of
    ...
    FoundMultiple _ → do
      addErr ("Found multiple modules named" <+> ppr mdName)
      return Nothing
    _ → do
      addErr ("Could not find a module named" <+> ppr mdName)
      return Nothing
